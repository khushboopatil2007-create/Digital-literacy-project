# 📚 Digital Literacy Portfolio
### CSE0001 – Digital Literacy | VIT Bhopal University

![Digital Literacy](https://img.shields.io/badge/Course-CSE0001%20Digital%20Literacy-blue?style=flat-square)
![Status](https://img.shields.io/badge/Status-Completed-success?style=flat-square)
![Grade](https://img.shields.io/badge/Grade-Pass%2FFail-yellow?style=flat-square)
![Year](https://img.shields.io/badge/Year-First%20Year-orange?style=flat-square)
![Branch](https://img.shields.io/badge/Branch-Integrated%20M.Tech%20AI-purple?style=flat-square)

---

## 👩‍🎓 Student Details

| Field | Details |
|---|---|
| 👤 **Name** | Khushboo Vinod Patil |
| 🎫 **Reg No.** | 25MIM10104 |
| 🎓 **Branch** | Integrated M.Tech – Artificial Intelligence |
| 📅 **Year** | First Year (Passout: 2030) |
| 🏫 **Institution** | VIT Bhopal University |
| 📘 **Course Code** | CSE0001 – Digital Literacy |
| ⭐ **Credits** | 1 Credit – Pass / Fail (Non-CGPA) |

---

## 🎯 About This Project

> *"As a Student Digital Ambassador, I built this Digital Literacy Portfolio to help my peers navigate the digital world safely, professionally, and responsibly."*

This repository is my complete **Digital Literacy Portfolio** for the CSE0001 course at VIT Bhopal University. It covers **five tasks** across five modules — from designing an infographic to understanding cybercrime.

🔑 **Key Highlights:**
- 🎨 Designed a Digital Literacy Infographic using Canva
- 💼 Set up professional profiles on GitHub, LinkedIn, and Kaggle
- 💻 Completed a coding challenge on HackerRank
- 📧 Drafted professional emails following email etiquette rules
- 🛡️ Researched UPI fraud and created a cybercrime prevention guide

> No prior coding knowledge was required. All tools used are free and browser-based.

---

## 📁 Repository Structure

```
digital-literacy-project/
│
├── 📄 README.md                              ← You are here
│
├── 📂 report/
│   └── 📝 Project_Report.pdf                ← Full project report
│
├── 📂 task-1-presentation/
│   ├── 📝 description.md                    ← Task description & reflection
│   └── 🖼️  digital_literacy_infographic.png ← Canva infographic export
│
├── 📂 task-2-portfolio/
│   ├── 📝 description.md                    ← Task description & reflection
│   ├── 🖼️  github_profile_screenshot.png
│   ├── 🖼️  linkedin_profile_screenshot.png
│   └── 🖼️  kaggle_profile_screenshot.png
│
├── 📂 task-3-platforms/
│   ├── 📝 description.md                    ← Task description & reflection
│   ├── 🖼️  hackerrank_challenge_screenshot.png
│   ├── 🖼️  google_form_screenshot.png
│   └── 🖼️  google_sheet_responses_screenshot.png
│
├── 📂 task-4-email-etiquette/
│   ├── 📝 description.md                    ← Task description & reflection
│   ├── 📄 email_drafts.pdf                  ← Both professional emails
│   └── 📝 social-media-checklist.md         ← Do's & Don'ts checklist
│
└── 📂 task-5-cybercrime/
    ├── 📝 description.md                    ← Task description & reflection
    ├── 📝 casestudy.md                      ← UPI fraud case study
    └── 📝 prevention-checklist.md           ← Stay Safe Online checklist
```

---

## 📋 Task Summaries

---

### ✅ Task 1 – Digital Literacy Awareness Infographic


Created a one-page **Digital Literacy Awareness Infographic** using **Canva** to help batchmates understand what digital literacy means and why it matters.

**Topics Covered:**
- 📖 What is Digital Literacy?
- 🔒 Safe Internet Practices
- 🌐 Professional Online Presence for Students

**Tool Used:** Canva (canva.com)
**🔗 Canva Design Link:** [ https://www.canva.com/design/DAHFLGcfyLo/ZfqPd99iaFHvUYUygUGZKg/edit?utm_content=DAHFLGcfyLo&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton]
**📁 Folder:** `task-1-presentation/`

---

### ✅ Task 2 – Student Digital Portfolio


Set up professional profiles on **three key platforms** to build a strong digital presence as an Integrated M.Tech AI student.

| Platform | Purpose | Profile Link |
|---|---|---|
| 🐙 **GitHub** | Code portfolio and project hosting | [ https://github.com/khushboopatil2007-create] |
| 💼 **LinkedIn** | Professional networking and career building | [ https://www.linkedin.com/in/khushboo-vinod-patil-59039b384?utm_source=share_via&utm_content=profile&utm_medium=member_android ] |
| 📊 **Kaggle** | Data science and AI competitions | [ https://www.kaggle.com/khushboo0919 ] |

**GitHub Profile README includes:**
- Name, Reg No., Branch, Year, Institution
- Learning Goal: To master Machine Learning and build real-world AI projects

**📁 Folder:** `task-2-portfolio/`

---

### ✅ Task 3 – Coding & Collaboration Platforms




**Part A – Google Workspace (Google Forms):**

Created a **"Digital Literacy Awareness Quiz"** with 5 questions for batchmates — including Multiple Choice and Short Answer questions.

Google Form Link [ https://docs.google.com/forms/d/e/1FAIpQLSeTKQrx8bCb4I8icwbfHhks7CPhfV7Wgr38MQxfulmrASJxjQ/viewform?usp=publish-editor]

**Part B ** **** – Coding Practice (HackerRank):**

| Detail | Info |
|---|---|
| Platform | HackerRank |
| Challenge | Solve Me First |
| Domain | Python / Problem Solving |
| Level | Beginner (Easy) |
| Result | ✅ Passed |
**🔗 Google Form Quiz Link:** [ https://docs.google.com/forms/d/e/1FAIpQLSeTKQrx8bCb4I8icwbfHhks7CPhfV7Wgr38MQxfulmrASJxjQ/viewform?usp=publish-editor ]
**📁 Folder:** `task-3-platforms/`

---

### ✅ Task 4 – Professional Email & Etiquette Guide

**Part A – Two Professional Emails drafted:**

| Email | From | To | Purpose |
|---|---|---|---|
| Email 1 | Student | Professor | Requesting assignment deadline extension |
| Email 2 | Student | Internship Coordinator | Expressing interest in summer internship |

**Part B – Social Media Checklist:**

Created a **Do's and Don'ts checklist** with 5+ Do's and 5+ Don'ts for responsible social media use as a college student.

**Top Rules:**
- ✅ Always think before you post
- ✅ Keep profiles professional and updated
- ❌ Never share your OTP or password online
- ❌ Never post personal location or home address publicly

**📁 Folder:** `task-4-email-etiquette/`

---

### ✅ Task 5 – Cybercrime Awareness Case Study & Prevention Guide
> 

**Part A – Case Study:**
Wrote a  case study** on **UPI / Online Payment Fraud** — covering how it happens step-by-step, who is targeted, and the real consequences for victims.

**Part B – Prevention Checklist:**
Created a **"Stay Safe Online" checklist** with **8+ actionable tips** for college students in India — including 2 UPI-specific financial safety tips.

**🔑 Most Important UPI Safety Rule:**
> You **NEVER** need to enter your UPI PIN to **RECEIVE** money.
> PIN is only required when you **SEND** money.

**🚨 Report Cybercrime Immediately:**

| Channel | Details |
|---|---|
| 🌐 Online Portal | [cybercrime.gov.in](https://cybercrime.gov.in) |
| 📞 Helpline | **1930** (24x7, Free, Pan-India) |

**📁 Folder:** `task-5-cybercrime/`

---

## 📊 Marks Summary

| Task | Description | Max Marks |
|---|---|---|
| Task 1 | Digital Literacy Infographic | |
| Task 2 | Student Digital Portfolio |  |
| Task 3 | Coding & Collaboration Platforms |  |
| Task 4 | Email Etiquette & Communication |  |
| Task 5 | Cybercrime Awareness Case Study |  |
| 🎁 Bonus | Repository Structure & README |  |


> ✅ **Pass Mark:** 50 / 100 | **Grade:** Pass / Fail | **Does NOT affect CGPA**

---

## 🛠️ Tools & Platforms Used

| Task | Tool | Purpose | Link |
|---|---|---|---|
| Task 1 | Canva | Infographic design | [canva.com]( https://www.canva.com/design/DAHFWFRUxPI/taufamy_lLrleyLQ9dsRwA/edit?utm_content=DAHFWFRUxPI&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton) |
| Task 2 | GitHub | Code portfolio hosting | [github.com](https://github.com/khushboopatil2007-create) |
| Task 2 | LinkedIn | Professional networking | [linkedin.com](https://www.linkedin.com/in/khushboo-vinod-patil-59039b384?utm_source=share_via&utm_content=profile&utm_medium=member_android) |
| Task 2 | Kaggle | AI/ML practice platform | [kaggle.com]( https://www.kaggle.com/khushboo0919) |
| Task 3 | HackerRank | Coding challenges | [hackerrank.com](https://hackerrank.com) |
| Task 3 | Google Forms | Quiz and data collection | [forms.google.com]( https://docs.google.com/forms/d/e/1FAIpQLSeTKQrx8bCb4I8icwbfHhks7CPhfV7Wgr38MQxfulmrASJxjQ/viewform?usp=publish-editor) |
| Task 5 | Cyber Crime Portal | Report cybercrime | [cybercrime.gov.in](https://cybercrime.gov.in) |

---

## 🗓️ My 5-Year Digital Growth Plan

| Year | Focus | Goals |
|---|---|---|
| Year 1 (2025–26) | Foundation | Set up GitHub, LinkedIn, Kaggle. Learn Python basics. |
| Year 2 (2026–27) | Building | Upload mini-projects, practice DSA on HackerRank. |
| Year 3 (2027–28) | Exploring | AI/ML projects, apply for internships via LinkedIn. |
| Year 4 (2028–29) | Specializing | Research work, Kaggle competitions, internship experience. |
| Year 5 (2029–30) | Graduating | Complete AI portfolio ready for placements in 2030. |

---

## 👩‍💻 About Me

Hi! I'm **Khushboo Vinod Patil** 👋

I am a first-year student pursuing **Integrated M.Tech in Artificial Intelligence** at VIT Bhopal University (Reg No: 25MIM10104), expected to graduate in **2030**.

I am passionate about **Artificial Intelligence and Machine Learning**, and I hope to build real-world AI projects over the next five years. This repository marks the beginning of my professional digital journey, and I plan to keep growing it as I learn and develop new skills.

📫 **Connect with me:**
- 🐙 GitHub: [ https://github.com/khushboopatil2007-create]
- 💼 LinkedIn: [ https://www.linkedin.com/in/khushboo-vinod-patil-59039b384?utm_source=share_via&utm_content=profile&utm_medium=member_android]
- 📊 Kaggle: [ https://www.kaggle.com/khushboo0919 ]

---

## 📜 Academic Integrity Statement

All written reflections, email drafts, case studies, and checklists in this repository are written in my own words and represent my own understanding. Screenshots are from my own accounts. This is an individual submission as per VIT Bhopal University guidelines.

---

*📘 Submitted as part of CSE0001 – Digital Literacy | VIT Bhopal University*
*👩‍🎓 Khushboo Vinod Patil | 25MIM10104 | Integrated M.Tech – AI | Passout: 2030*
